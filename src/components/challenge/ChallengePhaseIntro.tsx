...
export default function ChallengePhaseIntro() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[60dvh] px-6 text-center">
      ...
      }}>Phase {phase.phaseNum} of 5</span>

      <div className="flex gap-2 mt-3 mb-4">
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} style={{
            width: i < state.currentPhase ? 20 : 8, height: 4, borderRadius: 2,
            background: i < state.currentPhase ? "#0f4a58" : "var(--rule)", transition: "all 0.3s",
          }} />
        ))}
      </div>
      ...
      <button className="w-full max-w-xs cursor-pointer active:scale-[0.97] transition-transform" style={{
        background: "var(--ink)", color: "var(--bg)", border: "none",
        borderRadius: "var(--radius-sm)", padding: "14px 24px",
        fontSize: 14, fontWeight: 500, fontFamily: "var(--font-body)",
      }} onClick={actions.beginPhase}>
        Read the news
      </button>
    </div>
  );
