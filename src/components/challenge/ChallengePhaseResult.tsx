...
const QUALITY_CONFIG: { [key: string]: { bg: string; border: string; icon: string; color: string; label: string } } = {
...
export default function ChallengePhaseResult() {
  ...
  return (
    <div className="px-5 pb-8 pt-6">
      <span style={{ fontFamily: "var(--font-mono)", fontSize: 9, textTransform: "uppercase", letterSpacing: "0.14em", color: "var(--ink-4)" }}>
      ...
      <div className="mt-4 flex items-start gap-3" style={{
        ...
        <span className="shrink-0 flex items-center justify-center" style={{
          ...
          background: config.bg, color: config.color, fontSize: 13, fontWeight: 600,
        }}>{config.icon}</span>
        <div>
          ...
          <p style={{ fontSize: 14, color: "var(--ink-2)", lineHeight: 1.6 }}>{state.lastFeedback}</p>
        </div>
      </div>
        ...
        <div className="mt-2 flex flex-col gap-2">
          {Object.entries(state.scores).map(([key, val]) => (
            <div key={key} className="flex items-center gap-2.5">
              <span style={{ width: 80, fontFamily: "var(--font-mono)", fontSize: 9, textTransform: "uppercase", letterSpacing: "0.06em", color: "var(--ink-4)" }}>{key}</span>
              <div className="flex-1" style={{ height: 3, background: "var(--rule-light)", overflow: "hidden" }}>
                <div style={{ height: "100%", width: `${val}%`, background: val >= 60 ? "#0f4a58" : val >= 40 ? "var(--gold)" : "var(--coral)", transition: "width 0.6s ease" }} />
              </div>
              <span style={{ width: 24, textAlign: "right", fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--ink-3)" }}>{val}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {state.portfolioHistory.length > 1 && (
        <div className="mt-5 px-2 py-3" style={{ background: "var(--surface-dim)", border: "1px solid var(--rule-light)", borderRadius: "var(--radius-sm)" }}>
          <svg width="100%" height="40" viewBox="0 0 300 40">
            {(() => {
              const data = state.portfolioHistory;
              const min = Math.min(...data) * 0.95; const max = Math.max(...data) * 1.05;
              const range = max - min || 1;
              const points = data.map((v, i) => `${(i / Math.max(data.length - 1, 1)) * 300},${36 - ((v - min) / range) * 32}`).join(" ");
              return (
                <>
      ...
      <button className="mt-6 w-full cursor-pointer active:scale-[0.97] transition-transform" style={{
